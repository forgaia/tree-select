...
const MotionTreeNode: React.ForwardRefRenderFunction<CSSMotion, MotionTreeNodeProps> = (
  { className, style, motion, motionNodes, motionType, onMotionEnd, active, treeNodeRequiredProps, ...props },
  ref
) => {
        ...
        {({ className: motionClassName, style: motionStyle }, motionRef) => (
          <div ref={motionRef} className={classNames(`${prefixCls}-treenode-motion`, motionClassName)} style={motionStyle}>
            {motionNodes.map((treeNode: FlattenNode) => {
              ...
              return <TreeNode {...restProps} {...treeNodeProps} active={active} data={treeNode.data} key={key} isStart={isStart} isEnd={isEnd} />;
            })}
          </div>
        )}
      </CSSMotion>
