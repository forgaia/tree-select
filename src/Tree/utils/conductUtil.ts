...
// Fill miss keys
function fillConductCheck(keys: Set<Key>, levelEntities: Map<number, Set<DataEntity>>, maxLevel: number): ConductReturnType {
...
// Remove useless key
function cleanConductCheck(keys: Set<Key>, halfKeys: Key[], levelEntities: Map<number, Set<DataEntity>>, maxLevel: number): ConductReturnType {
  const checkedKeys = new Set<Key>(keys);
 ...
 */
export function conductCheck(keyList: Key[], checked: true | { checked: false; halfCheckedKeys: Key[] }, keyEntities: Record<Key, DataEntity>): ConductReturnType {
